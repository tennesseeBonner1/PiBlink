...
def openMatrixViewMenu():
    ...
    #(specify flags to remove "?" button on top of window, also removes "X" though, ughh)
...
def loadPageNoChecks(newPageNumber):
    ...
    matrixViewWrapper.trialNumbersLabel.setText("Showing trials " + str(startingTrial) + "-"
...
def getMatrixParameters(regenerating):
    ...
    else:   #Initially, Set the trial width and height defaults to current size of trial on graph
...
#Set display parameters to those currently saved in global variables
#Used when "change parameters" button is pressed to remember parameters
...
#Called when matrix view window is created to simplify the computation.
#Considers trialWidth, spacing, maxPageWidth, and maxColumns to determine...
#what the real maximum number of trials should be per row.
#Does the equivalent for columns too.
#The end result is the info being stored in global maxTrialsPerRow and maxTrialsPerColumn.
